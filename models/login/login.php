...
$otp_expires = $now->format('Y-m-d H:i:s');
session_start();
...
$stmt = $connect->prepare("SELECT * FROM admins WHERE email = ? AND status = 'Active'");
if (!$stmt) {
...
if ($data = $result->fetch_assoc()) {
    if (password_verify($password, $data['password'])) {
        ...
        $stmt = $connect->prepare("UPDATE admins SET otp_code = ?, otp_expires = ? WHERE id = ?");
        $stmt->bind_param('isi', $otp, $otp_expires, $data['id']);
        $stmt->execute();
        $stmt->close();
        ...
        try {
            ...
            $mail->Body = "
                ...
                <body>
                <div class='container'>
                    <h1>NewsToday OTP Verification</h1>
                    <p>Your one-time password (OTP) is valid for the next 5 minutes. Please use it to complete your login.</p>
        ...
        $_SESSION['response'] = ["msg" => "Login Failed", 'status' => false, "des" => "Wrong password"];
        header("Location: ../../views/login/index.php");
        exit();
    }
} else {
    $_SESSION['response'] = ["msg" => "Admin Not Found", 'status' => false, "des" => "Email didn't have an account or Account is now inactive."];
    header("Location: ../../views/login/index.php");
    exit();
}
?>